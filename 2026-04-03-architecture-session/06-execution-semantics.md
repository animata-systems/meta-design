...
Corazon Host — это единственный слой, где реально **происходит вычисление**. Все остальные слои занимаются хранением, распространением и легитимацией. Поэтому семантика исполнения — это ответ на вопрос «как события message-space превращаются в работу агента».
...
1. Хост подписан на релевантные header-логи.
2. Пришло новое header-событие. Хост проверяет: это событие в канале, на который я подписан?
3. Если да — проверяется локальное состояние членства: имеет ли этот хост (этот participant, обслуживаемый этим узлом) право читать канал в текущем epoch?
...
В двух первых шагах — фильтрация по релевантности, без раскрытия содержимого. В последующих — работа с plaintext, но только для того, кому реально адресовано. Ключевое свойство: хост **ничего лишнего не расшифровывает** и **ничего лишнего не хранит в plaintext**.
...
Это важная граница. Если один агент задаёт вопрос трём другим и ждёт ответа, это **не оркестрация на уровне ОС**. Это логика самого агента-инициатора. Хост не знает про scatter-gather — он знает только про «пришло событие, запусти инстанс, выполни шаг, положи состояние, жди следующего события».
...
Это снимает огромный класс операционных проблем: нечего «рестартить», нечего «мониторить как живой процесс», нечего «держать по 24/7». Есть только логи и хосты, которые по ним ходят.
...
```mermaid
flowchart TD
    A["Host listens to relevant header logs"] --> B["New message header arrives"]
...
Эта проекция показывает последовательность от **обнаружения заголовка** до **передачи входа в runtime**: проверка scope, проверка членства, резолв payload_ref, расшифровка, нормализация, доставка в инстанс. Видимость операционализируется на уровне хоста; никто не читает «всю систему».

```mermaid
flowchart TD
    A["Header event appears in subscribed log"] --> B["Host checks access scope"]
