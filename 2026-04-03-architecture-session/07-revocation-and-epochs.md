...
Аналогичная логика применяется к удалению отдельных сообщений. Автор может опубликовать `message_deleted` — ещё одно header-событие, которое говорит «считайте предыдущее сообщение удалённым». Sandboxes и UI **уважают** этот флаг и не показывают контент. Но сам header и сам payload при этом остаются в логе и в хранилище, потому что их оттуда не достать.
